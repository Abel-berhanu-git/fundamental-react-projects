...
##  cleanup function 
✅ in useEffect The cleanup function runs only in two cases:
...
##  the exact steps are:
 ...
 - First render:
1. Component renders
2. useEffect runs → creates interval
3. return () => clearInterval(...) is registered, but NOT executed
  
 - When dependency changes:
4. React runs cleanup from previous effect → clearInterval(...)
5. React runs the effect again → creates new interval

 - When component unmounts:
6. React runs cleanup one last time → clears interval